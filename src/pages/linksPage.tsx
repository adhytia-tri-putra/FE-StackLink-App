...
const IconGlobe = ({ className = "h-5 w-5", ...props }: IconProps) => (
  <svg
    ...
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M12 21a9 9 0 100-18 9 9 0 000 18zM3.6 9h16.8M3.6 15h16.8M12 3a14 14 0 010 18M12 3a14 14 0 000 18"
    />
  </svg>
);
...
const IconBag = ({ className = "h-5 w-5", ...props }: IconProps) => (
  <svg
    ...
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M8 8V7a4 4 0 018 0v1m-9 0h10l1 11H6L7 8z"
    />
  </svg>
);
...
const IconCalendar = ({ className = "h-5 w-5", ...props }: IconProps) => (
  <svg
    ...
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M8 7V4m8 3V4M5 11h14M6 5h12a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V7a2 2 0 012-2zm4 10h4"
    />
  </svg>
);
...
const IconTarget = ({ className = "h-5 w-5", ...props }: IconProps) => (
  <svg
    ...
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M12 3v3m0 12v3m9-9h-3M6 12H3m14.8-5.8l-2.1 2.1M8.3 15.7l-2.1 2.1m11.6 0l-2.1-2.1M8.3 8.3L6.2 6.2M12 16a4 4 0 100-8 4 4 0 000 8z"
    />
  </svg>
);
...
const IconEdit = ({ className = "h-5 w-5", ...props }: IconProps) => (
  <svg
    ...
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M16.862 4.487l1.65-1.65a1.875 1.875 0 112.651 2.651L8.25 18.401 4.5 19.5l1.099-3.75L16.862 4.487z"
    />
  </svg>
);
...
const IconExternal = ({ className = "h-4 w-4", ...props }: IconProps) => (
  <svg
    ...
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M13.5 6H18m0 0v4.5M18 6l-8.25 8.25M6 9v9h9"
    />
  </svg>
);
...
const IconTrash = ({ className = "h-5 w-5", ...props }: IconProps) => (
  <svg
    ...
    <path
      ...
      strokeLinejoin="round"
      strokeWidth="2"
      d="M6 7h12m-9 0V5a1 1 0 011-1h4a1 1 0 011 1v2m2 0l-.8 12a2 2 0 01-2 1.9H8.8a2 2 0 01-2-1.9L6 7z"
    />
  </svg>
);
...
const LinksPage: React.FC = () => {
    ...
    const initialize = async () => {
      ...
      try {
        ...
        const clicksById = new Map(
          summary.links.map((link) => [link.id, link.totalClicks]),
        );
        setLinks(
  ...
  const handleToggleActive = async (id: string, currentActive: boolean) => {
    setLinks((prev) =>
      prev.map((link) =>
        link.id === id ? { ...link, isActive: !currentActive } : link,
      ),
    );
  ...
  const saveOrder = async (
    nextLinks: LinkWithClicks[],
    fallbackLinks: LinkWithClicks[],
  ) => {
    setOrderStatus("Saving order...");

    try {
      await linkService.reorderLinks(
        nextLinks.map((link, index) => ({ id: link.id, position: index })),
      );
            ...
            {links.length === 0 ? (
              <div className="rounded-[20px] border border-outline-variant/60 bg-surface-container-lowest p-6 text-center shadow-soft">
                ...
                <p className="mx-auto mt-2 max-w-md text-on-surface-variant">
                  Create your first destination so your audience has somewhere
                  calm to land.
                </p>
                <button
                  onClick={() => setIsAddModalOpen(true)}
                      ...
                      {editingId === link.id ? (
                        ...
                        <div className="grid gap-3 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                          <div className="flex min-w-0 items-center gap-4">
                            ...
                            <span
                              className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full ${style.bg} ${style.text}`}>
                          ...
                          <div className="flex min-w-0 items-center justify-between gap-2 pl-[60px] sm:pl-0">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <button
                                ...
                                title={
                                  link.isActive ? "Pause link" : "Resume link"
                                }
